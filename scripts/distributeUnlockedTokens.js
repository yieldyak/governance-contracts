...
async function distributeUnlockedTokens() {
    ...
    const tokenName = await deployments.read('YakToken', 'name')
    const decimals = await deployments.read('YakToken', 'decimals')
    ...
    const domainSeparator = ethers.utils.keccak256(
        ethers.utils.defaultAbiCoder.encode(
          ...
          [DOMAIN_TYPEHASH, ethers.utils.keccak256(ethers.utils.toUtf8Bytes(tokenName)), ethers.utils.keccak256(ethers.utils.toUtf8Bytes("1")), ethers.provider.network.chainId, yakToken.address]
        )
      )

    const nonce = await deployments.read('YakToken', 'nonces', deployer)
